...
%LagLDDDM. 
%Only minimal changes to the original were made, which are all
%marked by the flag $CHANGE. 
% For details and license info concerning the original see
% - https://github.com/C4IR/FAIR.m/tree/master/add-ons/LagLDDMM.
%==============================================================================

% function  [Jc,para,dJ,H] = NCC_objFctn(T,Rc,z,omega,m,vRef,xc,omegaV,mV,N,vc,K,Kadj)
%
% Objective Function for LDDMM  whith source term using Lagrangian PDE solver 
%
% computes J(vc) = D(K(T(yc(vc)+z)),Rc) + S(vc - vRef), where
%
...
% K        = forward operator
% Kadj     = adjoint operator to K
% z        = discrete image on grid
...
%   z      - discrete image on grid (source term)
...
%  dJ      - gradient of J with respect to vc
%  H       - approximation to Hessian of J with respect to vc
%
% Note that, in difference to the LDDMMobjFct an additional source term z 
% is present.
%==============================================================================
function [Jc,para,dJ,H] = NCC_objFctn(T,Rc,z,omega,m,vRef,xc,omegaV,mV,N,vc,K,Kadj)
...
if(operator)
    ...
    %CHANGE : Add source term
    [Dc,~,dD,dres,dres_adj,d2psi] = dist(Tc+z,Rc,omega,m,K,Kadj,'doDerivative',true);
else
     %CHANGE : Add source term
    [Dc,~,dD,dres,d2psi] = distance(Tc+z,Rc,omega,m,'doDerivative',doDerivative);
end

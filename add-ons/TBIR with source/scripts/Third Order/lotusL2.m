%This example uses real data. The results can be found in Figure 5 g)f)
%(proposed method with L^2 regulariser)
...
outputfolder = fullfile(FAIRpath, 'add-ons', 'TBIR with source', 'results','lotus L2');
mkdir(outputfolder);
...
for i=1:length(A)
        a=A(i);
        name=append('lotus_',int2str(i));
    % Set parameters and models
    N = 5; % Runge-Kutta steps
    nt = 1; % Temporal discretization of velocity
    noise = 0.0; % Noise
    alpha = [0.01,0.01,1e-6]; % Regularization velocity.
    lambda = a; % Regularization source
    dist = 'NCC_op';
    reg = 'mfThirdOrderST';

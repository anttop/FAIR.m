% This script produces the results from Figure 2.f) (proposed method with
% L2-regularisation)
...
outputfolder = fullfile(FAIRpath, 'add-ons', 'TBIR with source', 'results','phantom L2');
mkdir(outputfolder);
...
noise = 0.05; % Noise
alpha = [0.001,0.001,1e-6]; % Regularization velocity.
%alpha = [100,100,1e-6]; % Regularization velocity.
lambda = 0.0001; % Regularization source
dist = 'SSD_op';
reg = 'mfThirdOrderST';
objfun = 'PALMobjFctn';
...
%PALM 
ProxG = @(x,sigma) proxTV_toolbox(x,sigma*lambda*m(1)); %proximal mapping for second variable

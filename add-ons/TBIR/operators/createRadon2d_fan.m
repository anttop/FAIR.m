...
function [K, Kadj, cleanup, ndet] = createRadon2d_fan(m, theta, ndet, scale, D, G)
%CREATERADON2D_FAN Creates function handles for 2D Radon transform using
%ASTRA Toolbox.
%
%   [K, Kadj, cleanup, ndet] = CREATERADON2D_FAN(m, theta, scale, D, G)
%   takes size of 2D geometry m, and angles of projections theta, and 
%   scaling factors, and returns function handles and the number of
%   detectors.
%
...
%   scale   a scalar > 0.
%   D, G    fan beam geometry settings.
%   
% Output:
...
proj_geom = astra_create_proj_geom('fanflat', 1.0, ndet, deg2rad(theta), D, G);

% Create operator function handles.
K = @(x) scale * radon2d_fan(reshape(x, m), proj_geom, vol_geom);
Kadj = @(x) reshape(radon2dadj_fan(scale * x, proj_geom, vol_geom), [], 1);
...
function sino = radon2d_fan(data, proj_geom, vol_geom)
...
function vol = radon2dadj_fan(sino, proj_geom, vol_geom)

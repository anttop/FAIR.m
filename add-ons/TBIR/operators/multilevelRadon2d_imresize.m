...
function ML = multilevelRadon2d_imresize(ML, maxLevel, minLevel)
%MULTILEVELRADON2D_IMRESIZE Creates multlevel versions of 2D Radon transform
%measurements using imresize.
%
%   ML = MULTILEVELRADON2D_IMRESIZE(ML, maxLevel, minLevel) takes a multilevel
%   cell array generated by getMultilevel and creates appropriate multilevel
%   versions of the 1D measurements generated by the 2D Radon transform by
%   simple downsampling.
%
% Input:
...
for k=1:maxLevel - minLevel
    ...
    ML{level}.R = imresize(data, [size(data, 1), ML{level}.ndet], 'bilinear', 'Antialiasing', false);
end
end

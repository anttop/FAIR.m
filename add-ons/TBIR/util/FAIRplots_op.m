...
function FAIRplots_op(task,varargin)
...
if strcmp(task,'reset') | strcmp(task,'set')
  ...
  defaults = {
    ...
    'plots',    1,...
    'position', [],...
    'Rshow',    @(R,omega,m) viewImage(255*R/max(R(:)),omega,m),...
    'Tshow',    @(T,omega,m) viewImage(255*T/max(T(:)),omega,m),...
    'Tname',    @(iter) sprintf('T(%d)',iter),...
    'Dshow',    @(T,R,omega,m) viewImage(reshape(T, m),omega,m),...
    'Dname',    @(j,Jc,Jstop) ...   
...
%     if isempty(fig), fig = figureh; else fig=figureh(fig); end;
    ...
    % check if array size of measurements is given
    if(exist('mR', 'var'))
        if(~isempty(mR))
            if(any(size(T)==1))
              T = reshape(T,m);
    ...
    figureh(fig); clf
      subplot(2,3,1); 
        plotOptn.R0handle = plotOptn.Rshow(R,omega,mR);      
